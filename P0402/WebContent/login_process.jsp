...
<% 
	...
	//DB 연동....(SELECT)
	String sql1 = String.format("SELECT * FROM FB_Member WHERE m_id = '%s' AND m_pw = '%s'",id1,pw1);
	// 문자열로 string타입이 두개가 들어가야함.
	System.out.println("실행될 쿼리 : " + sql1);
...
<%
	if(isSuccess){
		...
		out.println("<a href='./index.jsp'>메인 페이지로...</a><br>");
	}else{
		out.println("아이디 혹은 비밀 번호를 확인해 주세요...<br>");
		out.println("<a href='./index.jsp'>로그인 페이지로...</a><br>");
	}
